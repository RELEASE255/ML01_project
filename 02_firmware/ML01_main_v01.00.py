...
# -*- coding: utf-8 -*-
"""
ML01 project by RELEASE255 is a DIY LED board based on the Raspberry Pi Pico 2W microcontroller
Copyright (c) RELEASE255 | All rights reserved
MIT License
-----------------------------------------------------------------------------------------------
- FILE NAME: ML01_main_v01.00.py
- DESCRIPTION: Firmware written in MicroPython allowing the ML01 device to operate autonomously
- USE: rename to main.py for use with the index.html and microdot.py files on ML01 device
- HARDWARE COMPATIBILITY: PCB ML01v01 RevA & RevB
-----------------------------------------------------------------------------------------------
VERSION HISTORY:
- 2026.01.17 version 01.00 - Initial version
-----------------------------------------------------------------------------------------------
"""
...
# WiFi
WIFI_SSID = "yourSSID"  # SSID of the router to which the pico needs to connect
WIFI_PASSWORD = "yourPassword"  # Password of the router to which the pico needs to connect
WIFI_CHECK_INTERVAL = 600
WIFI_RECONNECT_ATTEMPTS = 2
WIFI_RECONNECT_DELAY = 20

# NTP
NTP_TIMEZONE_OFFSET = 1  # Choose your time zone here (UTC+1 = 1, UTC+2 = 2, etc.)

# Server
WEB_PORT = 80
DEVICE_NAME = "ML01 project by RELEASE255"
...
# Timing
CHASE_SPEED = 3.75  # Lighting duration of each LED in CHASE mode -> 60 minutes / 16 LED = 3.75 (in minutes)
CHASE_ANIMATION_SPEED = 0.05  # Duration of each LED's illumination in CHASE mode during the special hourly animation (in seconds)
CHASE_BLINK_SPEED = 1  # Blink speed for the special hourly animation finale (in seconds)
DEBOUNCE_TIME = 50
LONG_PRESS_TIME = 1500

# Authentication
AUTH_ENABLED = True
AUTH_USERNAME = "admin"  # Choose the login for remote control with the microdot server
AUTH_PASSWORD = "ML01secure!"  # Choose the password for remote control with the microdot server
AUTH_TOKEN = None

# Security
IP_WHITELIST_ENABLED = True
ALLOWED_IPS = [
    "192.168.x.xxx",  # Your computer
    "192.168.x.xxx",  # Your phone
    "192.168.x.xxx"   # Another device
]  # Edit above the IP addresses authorized for remote control with the microdot server
