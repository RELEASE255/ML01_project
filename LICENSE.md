# ML01 project — License Overview

This project uses multiple licenses for different categories of files.
The main license is MIT (see the `LICENSE` file), but other parts of the project use Creative Commons licenses.

Below is the complete list of licenses used in the repository.

---

## Documentation (PDF)
- **License:** CC BY-NC-SA 4.0  
- **Files:** `01_docs/*.pdf`  
- **Full text:** [01_docs/LICENSE_docs.md](01_docs/LICENSE_docs.md)

---

## Firmware (Python + HTML)
- **License:** MIT  
- **Files:** `02_firmware/*.py`, `02_firmware/*.html`  
- **Full text:** See the `LICENSE` file (root)

---

## Microdot (embedded library)
- **License:** MIT  
- **Files:** `02_firmware/microdot.py`  
- **Full text:** [02_firmware/LICENSE_microdot](02_firmware/LICENSE_microdot)

---

## MicroPython References
- **License:** MIT  
- **Files:** Any file derived from MicroPython  
- **Full text:** [02_firmware/LICENSE_micropython](02_firmware/LICENSE_micropython)

---

## Hardware
- **License:** CC BY-NC-SA 4.0  
- **Files:** `03_hardware/*.pdf`  
- **Full text:** [03_hadware/LICENSE_hardware.md](03_hadware/LICENSE_hardware.md)

---

## Bill of Materials (BOM)
- **License:** CC BY-SA 4.0  
- **Files:** `03_hardware/*.ods`  
- **Full text:** [03_hadware/LICENSE_bom.md](03_hadware/LICENSE_bom.md)

---

## 3D Printed Parts (STL / STEP)
- **License:** CC BY-NC-SA 4.0  
- **Files:** `04_3d/*.stl`, `04_3d/*.stp`  
- **Full text:** [04_3d/LICENSE_3d.md](04_3d/LICENSE_3d.md)

---

## Images & Photos
- **License:** CC BY-NC-SA 4.0  
- **Files:** `05_images/**/*.jpg`, `05_images/**/*.png`  
- **Full text:** [05_images/LICENSE_images.md](05_images/LICENSE_images.md)

---

If you have any question about licensing or permitted uses, feel free to open an issue or contact the project maintainer.
© 2025 ML01 project by RELEASE255 — All rights reserved where applicable.
